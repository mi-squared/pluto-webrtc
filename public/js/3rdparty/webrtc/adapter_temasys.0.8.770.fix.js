    ...
    attachMediaStream = function(element, stream) {
      ...
      if (element.nodeName.toLowerCase() !== 'audio') {
        ...
        if (!element.isTemWebRTCPlugin || !element.isTemWebRTCPlugin()) {
          ...
          // Получить размеры исходного элемента
          var rectObject = element.getBoundingClientRect();

          // Вставить новый элемент перед существующим
          element.parentNode.insertBefore(frag, element);

          // Получить ???текущий элемент по его ID ???
          ...
          console.log('attachMediaStream()' +
            ': frag.nodeName', frag.nodeName,
            '; frag.id:', frag.id);

          console.log('attachMediaStream()', frag.width, frag.height);
